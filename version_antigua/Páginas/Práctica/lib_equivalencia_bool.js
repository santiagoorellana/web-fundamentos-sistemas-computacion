/* 
Nombre=lib_equivalencia_bool.js
...
while(n<y[0]){                          //esto se repite infinitamente
     ...
     for(n=1;n<=y[0];n++){              //busca un operador X.
        if(y[n]==x){                    //si lo encuentra, lo ejecuta.
          if(n==y[0])error(8,1);else{   //error si no hay segundo operando
...
//---------------------------------------------------------
function teclas1(x){
document.getElementById("pantalla1").focus();
var y=document.getElementById("pantalla1").value;
if((x>='A')&&(x<='H'))
  document.getElementById("pantalla1").value=y+x
else
  if((x=='0')||(x=='1')||(x=='(')||(x==')'))
    document.getElementById("pantalla1").value=y+x;
  else
    if(x=='·')document.getElementById("pantalla1").value=y+'·';
    else
    if(x=='+')document.getElementById("pantalla1").value=y+'+';
    else
    if(x=='-')document.getElementById("pantalla1").value=y+"'";
    else 
    if(x=='L')document.getElementById("pantalla1").value='';
    else 
    if(x=='R'){
      y=document.getElementById("pantalla1").value;
      document.getElementById("pantalla1").value=y.substring(0,y.length-1);}
    else
    if(x=='=')comprobar_equivalencia(document.getElementById("pantalla1").value,document.getElementById("pantalla2").value);
};

//---------------------------------------------------------
function teclas2(x){
document.getElementById("pantalla2").focus();
var y=document.getElementById("pantalla2").value;
if((x>='A')&&(x<='H'))
  document.getElementById("pantalla2").value=y+x
else
  if((x=='0')||(x=='1')||(x=='(')||(x==')'))
    document.getElementById("pantalla2").value=y+x;
  else
    if(x=='·')document.getElementById("pantalla2").value=y+'·';
    else
    if(x=='+')document.getElementById("pantalla2").value=y+'+';
    else
    if(x=='-')document.getElementById("pantalla2").value=y+"'";
    else 
    if(x=='L')document.getElementById("pantalla2").value='';
    else 
    if(x=='R'){
      y=document.getElementById("pantalla2").value;
      document.getElementById("pantalla2").value=y.substring(0,y.length-1);};
};

//---------------------------------------------------------
function comprobar_variables(x,y){
if(x[0]!=y[0]){
  responder('NO SE PUEDE REALIZAR LA COMPARACIÓN\nLas expresiones introducidas no tienen la misma cantidad de variables.');
  return 1;}
else
  for(var n=1;n<=x[0];n++)
     if(x[n]!=y[n]){
       responder('NO SE PUEDE REALIZAR LA COMPARACIÓN\nLas expresiones introducidas tienen diferentes variables.');
       return 1;
       };
return 0;
};
...
//---------------------------------------------------------
function comprobar_equivalencia(x,y){
vx=x;
vy=y;
if((x=='')||(y=='')){
  alert('Deben introducirse dos expresiones para comprobar su equivalencia.');
  err=1;
  return 0;
  };
x=array_to_string(borra_espacio(string_to_array(x)));
y=array_to_string(borra_espacio(string_to_array(y)));
...
err_mensajes=errores(y);
if(err_mensajes!=''){
  alert(err_mensajes);
  err=1;
  return 0;
  };
var exp_bool_x=array_to_string(simple_to_code(string_to_array(x)));
var exp_bool_y=array_to_string(simple_to_code(string_to_array(y)));
exp_bool_x=string_to_array(exp_bool_x);
exp_bool_y=string_to_array(exp_bool_y);
var v_encontradas_x=ordenar(busca_var(exp_bool_x));
var v_encontradas_y=ordenar(busca_var(exp_bool_y));
if(comprobar_variables(v_encontradas_x,v_encontradas_y)==1){
  return 0;
  };
if(v_encontradas_x[0]>8)
...
n=Math.pow(2,v_encontradas_x[0])-1; 
exp_bool_x=array_to_string(exp_bool_x); 
exp_bool_y=array_to_string(exp_bool_y); 
err=0;
var tx=new Array();
    ty=new Array();
for(n;n>=0;n--){
   tx[n]=eval_exp(asig_val(exp_bool_x,n,v_encontradas_x));
   if(err==1)return 0;
   ty[n]=eval_exp(asig_val(exp_bool_y,n,v_encontradas_y));
   if(err==1)return 0;
   if(tx[n]!=ty[n]){
     responder('NO SON EQUIVALENTES\nLas tablas booleanas de las expresiones introducidas, son diferentes.');
     return 0;
     };
   };
responder('SON EQUIVALENTES\nAmbas expresiones se representan mediante la misma tabla booleana.\n\n'+vx+' = '+vy);
};
...
tb.document.getElementById("text_tabla").value=x+'\n';   	
};
